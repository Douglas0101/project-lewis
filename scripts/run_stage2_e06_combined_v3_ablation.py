"""Run E06R-H8: combined causal RR + N/V/F templates + N/V hybrid projection."""
...
LOGGER = logging.getLogger("run_stage2_e06_combined_v3_ablation")
FEATURE_SETS = ("baseline16", "h8_combined_v3")


class H8RunConfig(E06RunConfig):
    """Serializable H8 configuration."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    full_npz: Path
    full_parquet: Path
    template_config: FusionTemplateConfig = FusionTemplateConfig()
    class_template_config: FusionClassTemplateConfig = FusionClassTemplateConfig()


class H8Verdict(BaseModel):
    """Machine-readable H8 outcome."""
...
def _save_h2_state(
    ...
    fold_dir = output_dir / "templates_h2" / f"fold_{fold_index + 1}"
...
def _save_h5_state(
    ...
    fold_dir = output_dir / "templates_h5" / f"fold_{fold_index + 1}"
    fold_dir.mkdir(parents=True, exist_ok=True)
    (fold_dir / f"{scope}_state.json").write_text(
        json.dumps(transformer.state_manifest(), indent=2) + "\n",
        encoding="utf-8",
...
def _fit_h2_transformer(
    ...
    transformer = FusionTemplateTransformer(config.template_config)
...
def _fit_h5_transformer(
    ...
    transformer = FusionClassTemplateTransformer(config.class_template_config)
    return transformer.fit(
        full_signals,
        full_labels,
        full_groups,
...
def _verdict(
    ...
    return H8Verdict(
        hypothesis=(
            "Combined causal RR context, N/V/F class templates and N/V hybrid projection "
            "materially improve patient-wise fusion-beat classification outside records 208/213."
        ),
        result=(
            "REPRESENTATION_HYPOTHESIS_SUPPORTED" if supported else "H8_COMBINED_V3_NOT_SUFFICIENT"
        ),
...
def run(config: H8RunConfig) -> dict[str, Any]:
    """Execute paired baseline/H8 folds with inner and outer states."""
    ...
    h2_schema = build_template_feature_schema(config.template_config)
    (config.output_dir / "h2_feature_schema.json").write_text(
        h2_schema.model_dump_json(indent=2) + "\n",
        encoding="utf-8",
    )

    h5_schema = build_class_template_feature_schema(config.class_template_config)
    ...
    for fold_index, (outer_train, outer_test) in enumerate(outer_splits):
        ...
        inner_candidate = np.column_stack(
            [
                base_features,
                h3_context,
                inner_h2.transform(stage2_signals),
                inner_h5.transform(stage2_signals),
            ]
        ).astype(np.float32, copy=False)
        _save_h2_state(config.output_dir, fold_index, "inner_train", inner_h2)
        _save_h5_state(config.output_dir, fold_index, "inner_train", inner_h5)

        outer_h2 = _fit_h2_transformer(
            config,
            full_signals,
            full_labels,
            full_groups,
        ...
        outer_candidate = np.column_stack(
            [
                base_features,
                h3_context,
                outer_h2.transform(stage2_signals),
                outer_h5.transform(stage2_signals),
            ]
        ).astype(np.float32, copy=False)
        _save_h2_state(config.output_dir, fold_index, "outer_train", outer_h2)
        _save_h5_state(config.output_dir, fold_index, "outer_train", outer_h5)
        ...
        for seed in config.seeds:
            ...
            for feature_set, best_epoch, predictions, probabilities in (
                ...
                ("h8_combined_v3", candidate_epoch, candidate_pred, candidate_prob),
            ):
    ...
    manifest = {
        ...
        "candidate_feature_names": (
            ...
            + list(TEMPLATE_FEATURE_NAMES)
            + list(CLASS_TEMPLATE_FEATURE_NAMES)
        ),
        "git": _git_identity(),
        "verdict": verdict.model_dump(mode="json"),
    }
    (config.output_dir / "E06R_H8_manifest.json").write_text(
...
def main() -> int:
    ...
    summary = run(config)
    LOGGER.info("E06R-H8 verdict: %s", summary["verdict"])
    return 0
