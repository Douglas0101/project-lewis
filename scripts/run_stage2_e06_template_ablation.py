"""Run E06R-H2: fold-local N/V templates and hybrid projection."""
...
LOGGER = logging.getLogger("run_stage2_e06_template_ablation")
FEATURE_SETS = ("baseline16", "h2_fold_local_templates")


class H2RunConfig(E06RunConfig):
    """Serializable H2 configuration."""
...
class H2Verdict(BaseModel):
    """Machine-readable H2 outcome."""
    ...
    result: Literal[
        ...
        "H2_FOLD_LOCAL_TEMPLATES_NOT_SUFFICIENT",
    ]
...
def _verdict(
    ...
    return H2Verdict(
        hypothesis=(
            "Fold-local N/V template banks and hybrid projection materially improve "
            "patient-wise fusion-beat classification outside records 208/213."
        ),
        result=(
            "REPRESENTATION_HYPOTHESIS_SUPPORTED"
            if supported
            else "H2_FOLD_LOCAL_TEMPLATES_NOT_SUFFICIENT"
        ),
...
def run(config: H2RunConfig) -> dict[str, Any]:
    """Execute paired baseline/H2 folds with inner and outer template states."""
    ...
    schema = build_template_feature_schema(config.template_config)
    ...
    for fold_index, (outer_train, outer_test) in enumerate(outer_splits):
        ...
        inner_template_features = inner_transformer.transform(stage2_signals)
        inner_candidate = np.column_stack([base_features, inner_template_features]).astype(
        ...
        outer_template_features = outer_transformer.transform(stage2_signals)
        outer_candidate = np.column_stack([base_features, outer_template_features]).astype(
            np.float32, copy=False
        )
        _save_template_state(
            config.output_dir,
    ...
    candidate_summary = _aggregate_feature_set(
        predictions_frame.loc[feature_sets == "h2_fold_local_templates"]
    )
    verdict = _verdict(baseline_summary, candidate_summary, config)
    summary = {
        "baseline16": baseline_summary,
        "h2_fold_local_templates": candidate_summary,
        "delta": {
    ...
    manifest = {
        "stage": "E06_REOPENED",
        "hypothesis_id": "H2_FOLD_LOCAL_NV_TEMPLATES",
        ...
        "candidate_feature_names": list(BASE_FEATURE_NAMES) + list(TEMPLATE_FEATURE_NAMES),
        "git": _git_identity(),
        "verdict": verdict.model_dump(mode="json"),
    }
    (config.output_dir / "E06R_H2_manifest.json").write_text(
...
def main() -> int:
    ...
    summary = run(config)
    LOGGER.info("E06R-H2 verdict: %s", summary["verdict"])
    return 0
