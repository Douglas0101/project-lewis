...
def main():
    ...
    eval_result = evaluate_fold(
        model, X_test_norm, y_test, class_names=["N", "Anormal"]
    )
    ...
    LOGGER.info("Fold %d unfrozen | AUC=%.4f | F1_macro=%.4f | Acc=%.4f", fold_idx, auc, eval_result["global"]["F1_macro"], eval_result["global"]["Acc"])

    out = Path("experiments/smoke_stage1_unfrozen")
