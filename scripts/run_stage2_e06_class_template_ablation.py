"""Run E06R-H5: fold-local N/V/F class-template features."""
...
LOGGER = logging.getLogger("run_stage2_e06_class_template_ablation")
FEATURE_SETS = ("baseline16", "h5_class_templates")


class H5RunConfig(E06RunConfig):
    """Serializable H5 configuration."""
...
class H5Verdict(BaseModel):
    """Machine-readable H5 outcome."""
    ...
    result: Literal[
        ...
        "H5_CLASS_TEMPLATES_NOT_SUFFICIENT",
    ]
...
def _verdict(
    ...
    return H5Verdict(
        hypothesis=(
            "Fold-local N/V/F class-template banks materially improve patient-wise "
            "fusion-beat classification outside records 208/213."
        ),
        result=(
            "REPRESENTATION_HYPOTHESIS_SUPPORTED"
            if supported
            else "H5_CLASS_TEMPLATES_NOT_SUFFICIENT"
        ),
...
def run(config: H5RunConfig) -> dict[str, Any]:
    """Execute paired baseline/H5 folds with inner and outer template states."""
    ...
    schema = build_class_template_feature_schema(config.template_config)
    ...
    for fold_index, (outer_train, outer_test) in enumerate(outer_splits):
        ...
        for seed in config.seeds:
            ...
            for feature_set, best_epoch, predictions, probabilities in (
                ...
                ("h5_class_templates", candidate_epoch, candidate_pred, candidate_prob),
            ):
    ...
    candidate_summary = _aggregate_feature_set(
        predictions_frame.loc[feature_sets == "h5_class_templates"]
    )
    verdict = _verdict(baseline_summary, candidate_summary, config)
    summary = {
        "baseline16": baseline_summary,
        "h5_class_templates": candidate_summary,
        "delta": {
    ...
    manifest = {
        "stage": "E06_REOPENED",
        "hypothesis_id": "H5_CLASS_TEMPLATES",
        ...
        "candidate_feature_names": list(BASE_FEATURE_NAMES) + list(CLASS_TEMPLATE_FEATURE_NAMES),
        "git": _git_identity(),
        "verdict": verdict.model_dump(mode="json"),
    }
    (config.output_dir / "E06R_H5_manifest.json").write_text(
...
def main() -> int:
    ...
    summary = run(config)
    LOGGER.info("E06R-H5 verdict: %s", summary["verdict"])
    return 0
