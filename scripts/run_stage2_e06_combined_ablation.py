"""Run E06R-H6: combined causal RR context + N/V/F class templates."""
...
LOGGER = logging.getLogger("run_stage2_e06_combined_ablation")
FEATURE_SETS = ("baseline16", "h6_combined")


class H6RunConfig(E06RunConfig):
    """Serializable H6 configuration."""
...
class H6Verdict(BaseModel):
    """Machine-readable H6 outcome."""
...
def _verdict(
    ...
    return H6Verdict(
        hypothesis=(
            "Combined causal RR context and N/V/F class templates materially improve "
            "patient-wise fusion-beat classification outside records 208/213."
        ),
        result=(
            "REPRESENTATION_HYPOTHESIS_SUPPORTED" if supported else "H6_COMBINED_NOT_SUFFICIENT"
        ),
...
def run(config: H6RunConfig) -> dict[str, Any]:
    """Execute paired baseline/H6 folds with inner and outer states."""
    ...
    # Precompute H3 causal RR features (stateless, same for all folds)
    ...
    for fold_index, (outer_train, outer_test) in enumerate(outer_splits):
        ...
        inner_candidate = np.column_stack([base_features, h3_context, inner_h5]).astype(
        ...
        outer_candidate = np.column_stack([base_features, h3_context, outer_h5]).astype(
            np.float32, copy=False
        )
        ...
        for seed in config.seeds:
            ...
            for feature_set, best_epoch, predictions, probabilities in (
                ...
                ("h6_combined", candidate_epoch, candidate_pred, candidate_prob),
            ):
    ...
    candidate_summary = _aggregate_feature_set(predictions_frame.loc[feature_sets == "h6_combined"])
    verdict = _verdict(baseline_summary, candidate_summary, config)
    summary = {
        "baseline16": baseline_summary,
        "h6_combined": candidate_summary,
        "delta": {
    ...
    (config.output_dir / "E06R_H6_manifest.json").write_text(
...
def main() -> int:
    ...
    summary = run(config)
    LOGGER.info("E06R-H6 verdict: %s", summary["verdict"])
    return 0
