"""Run E06R-H7: combined causal RR + N/V/F templates + sampled QRS."""
...
LOGGER = logging.getLogger("run_stage2_e06_combined_v2_ablation")
FEATURE_SETS = ("baseline16", "h7_combined_v2")


class H7RunConfig(E06RunConfig):
    """Serializable H7 configuration."""
    ...
    sampled_config: SampledQRSConfig = SampledQRSConfig()


class H7Verdict(BaseModel):
    """Machine-readable H7 outcome."""
...
def _verdict(
    ...
    return H7Verdict(
        hypothesis=(
            "Combined causal RR context, N/V/F class templates and sampled QRS shape "
            "materially improve patient-wise fusion-beat classification outside records 208/213."
        ),
        result=(
            "REPRESENTATION_HYPOTHESIS_SUPPORTED" if supported else "H7_COMBINED_V2_NOT_SUFFICIENT"
        ),
...
def run(config: H7RunConfig) -> dict[str, Any]:
    """Execute paired baseline/H7 folds with inner and outer states."""
    ...
    h4_features = extract_sampled_qrs_morphology(stage2_signals, config.sampled_config)
    h4_schema = build_sampled_qrs_schema(config.sampled_config)
    (config.output_dir / "h4_feature_schema.json").write_text(
        h4_schema.model_dump_json(indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    for fold_index, (outer_train, outer_test) in enumerate(outer_splits):
        ...
        inner_candidate = np.column_stack(
            [base_features, h3_context, h4_features, inner_h5]
        ...
        outer_candidate = np.column_stack(
            [base_features, h3_context, h4_features, outer_h5]
        ).astype(np.float32, copy=False)
        ...
        for seed in config.seeds:
            ...
            for feature_set, best_epoch, predictions, probabilities in (
                ...
                ("h7_combined_v2", candidate_epoch, candidate_pred, candidate_prob),
            ):
    ...
    manifest = {
        ...
        "candidate_feature_names": (
            ...
            + list(feature.name for feature in h4_schema.features)
            + list(CLASS_TEMPLATE_FEATURE_NAMES)
        ),
        "git": _git_identity(),
        "verdict": verdict.model_dump(mode="json"),
    }
    (config.output_dir / "E06R_H7_manifest.json").write_text(
...
def main() -> int:
    ...
    summary = run(config)
    LOGGER.info("E06R-H7 verdict: %s", summary["verdict"])
    return 0
